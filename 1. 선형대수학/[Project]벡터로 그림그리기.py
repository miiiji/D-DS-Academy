...
def new(P):
    def new_diamond(P):
        dia = P - np.array([-0.5, 0.5])
        return np.sqrt(np.sum(dia*dia)) - 0.1
    
    return circle(P) * new_diamond(P)
...
accepted_points, rejected_points = sample(
                                          ...
                                          xrange,
                                          yrange,
                                          new, # smile을 circle 이나 diamond 로 바꿔서 실행해 보세요.
                                          0.001) # Threshold를 0.01이나 0.0001 같은 다른 값으로 변경해 보세요.
